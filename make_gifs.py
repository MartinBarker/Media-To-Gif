...
def make_gif(
    ...
    """
    #get random episode and subs
    gifDetails = getDetails()

    video_path = gifDetails['vidLocation']
    subs_path = gifDetails['subsLocation']
    episodeName = gifDetails['vidName']
    ...
    for working_index in sub_index:
        ...
        loop += 1
    print('subtitle.start = ', subtitle.start)
    subsStartStr = str(subtitle.start)
    subsStartStr = subsStartStr.partition(',')[0]
    print('subsStartStr = ', subsStartStr)
    # create a fuckin' gif
...
def make_gif_old(movie_slug, sub_index=[-1], custom_subtitle=[""], quote=True,
            frames=0, filename="star_wars2.gif", dither=DITHER,
...
if __name__ == '__main__':
    ...
    make_gif_old(random.choice(args.movie), sub_index=args.index, quote=args.quote,
